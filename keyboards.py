...
def admin_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="Блокировки времени", callback_data="admin_unavailable")],
        [InlineKeyboardButton(text="Назад", callback_data="main_menu")],
    ])
...
def admin_services_kb() -> InlineKeyboardMarkup:
    ...
    for i in range(0, len(service_items), 2):
        ...
        for service, price in service_items[i:i+2]:
            display = service[:14] + "…" if len(service.encode("utf-8")) > 28 else service
            row.append(InlineKeyboardButton(
                text=f"{display} — {price:,} ₸ / {config.get_service_duration(service)} мин".replace(",", " "),
...
def admin_unavailable_kb(periods: list[dict]) -> InlineKeyboardMarkup:
    ...
    for period in periods[:10]:
        label = f"Удалить #{period['id']} — {period['date']} {period['start_time']}-{period['end_time']}"
        rows.append([InlineKeyboardButton(text=label[:60], callback_data=f"admin_delete_unavailable:{period['id']}")])
...
def admin_cancel_booking_kb(booking_id: str, telegram_id: int | None = None, user_blocked: bool = False) -> InlineKeyboardMarkup:
    ...
    if telegram_id:
        ...
        text = "Разблокировать клиента" if user_blocked else "Заблокировать клиента"
...
def portfolio_kb(photo_id: int, has_prev: bool, has_next: bool, links: list[dict]) -> InlineKeyboardMarkup:
    """Navigation keyboard for portfolio gallery plus social links."""
    rows = []
    nav_row = []
    if has_prev:
...
def social_links_admin_kb(links: list[dict]) -> InlineKeyboardMarkup:
    ...
    for link in links:
        ...
        buttons.append([InlineKeyboardButton(
            text=f"{platform} — {link['url'][:40]}",
            callback_data=f"admin_delete_social_link:{link['id']}"
        )])
    buttons.append([
        InlineKeyboardButton(text="Добавить ссылку", callback_data="admin_add_social_link"),
        InlineKeyboardButton(text="Назад", callback_data="admin"),
    ])
    return InlineKeyboardMarkup(inline_keyboard=buttons)
